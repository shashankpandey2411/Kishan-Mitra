...
import os
import numpy as np
from tensorflow.keras.preprocessing import image  # Use tensorflow.keras instead of keras
from tensorflow.keras.models import load_model  # Use tensorflow.keras instead of keras
import pickle

# Load the Keras model with TensorFlow backend
...
def predict():
    if request.method == 'POST':
        ...
        # Check if pred is an array and not a scalar
        if isinstance(pred, np.ndarray):
            pred = pred[0]  # Access the first element if it's an array
